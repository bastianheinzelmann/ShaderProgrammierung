...
// camera
QuatCamera camera(glm::vec3(0.0f, 0.0f, 3.0f));
...
int main()
{
	...
	for (int i = 0; i < 192; i+=2)
	{
		for (int j = 0; j < 96; j++)
		{
			uvCoordinates[i + j * i] = 1.0f / 96.0f * i / 2;
			uvCoordinates[(i + j * i) + 1] = 1.0f / 96.0f * j;
		}
	}

	unsigned int uvVAO, uvVBO;
	...
	for (int i = 0; i < 256; i++)
	{
		noiseShader.setInt("layer", i);
		glad_glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, texture3d, 0, i);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		screenShader.use();
		glBindVertexArray(quadVAO);
		glBindTexture(GL_TEXTURE_3D, texture3d);	// use the color attachment texture as the texture of the quad plane
		glDrawArrays(GL_TRIANGLES, 0, 6);
